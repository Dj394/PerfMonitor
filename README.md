...
### Empreinte
Mesurée sur un Ryzen 5800X : **≈ 0,1 % CPU et ~60 Mo de RAM** réduit dans la zone de notification (≈ 1 % / 170 Mo fenêtre ouverte, à cause du rendu des graphiques). Sur un portable i7-11800H avec un écran 3072×1920 : **0,12 % CPU et ~80 Mo** en zone de notification, ~180 Mo fenêtre ouverte — le rendu WPF suit la définition de l'écran, et la fenêtre ne rend qu'une partie de sa mémoire quand on la referme (l'application reste stable dans le temps, sans fuite). Processus lus par l'API .NET toutes les 5 s, capteurs carte mère toutes les 5 s, SMART toutes les 30 s. La mesure elle-même ne lit que des compteurs noyau (`GetSystemTimes`, `GlobalMemoryStatusEx`) : tout ce qui peut être lent — WMI (disques, réseau, pression mémoire, batterie, score de redémarrage), énumération des processus, capteurs LibreHardwareMonitor et SMART — tourne sur des threads séparés qui publient leur dernière valeur. Sans ça, une machine saturée faisait tomber la cadence à 20-60 s (WMI mesuré à 19 s pour une seule requête) et les alertes à maintien ne partaient pas au moment où elles servent ; un trou de mesure est désormais journalisé dans `data\live.log`. **Mode économie automatique** (Paramètres, désactivable) : 1 mesure toutes les 2 s sur batterie ou fenêtre fermée depuis > 5 min — l'historique 5 s, les alertes et les rapports ne changent pas. La ligne « Coût de PerfMonitor » de l'onglet Machine affiche la consommation en direct.
